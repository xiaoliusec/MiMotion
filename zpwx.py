...
def login_access_token(user, password):
    ...
    r1 = requests.post(
        url1, data=cipher_data, headers=headers, allow_redirects=False, timeout=5
    )
...
def grant_login_tokens(access_token, device_id, is_phone=False):
    ...
    try:
        ...
        return None, None, None, "提取login_token失败：%s" % resp
...
def main():
    print("=" * 50)
    print("小米运动/Zepp Life 步数修改工具")
    print("=" * 50)
    print()

    user = input("请输入小米运动账号（邮箱或手机号）: ").strip()
    password = input("请输入密码: ").strip()
    step = input("请输入要设置的步数: ").strip()
    ...
    print()
    print(f"[{format_now()}] 账号：{desensitize_user(user)}")
    print(f"[{format_now()}] 目标步数：{step}")
    print(f"[{format_now()}] 开始登录...")

    device_id = str(uuid.uuid4())
    print(f"[{format_now()}] 设备ID：{device_id}")

    if not user.startswith("+86"):
        if "@" in user:
            is_phone = False
            print(f"[{format_now()}] 登录类型：邮箱登录")
        else:
            user = "+86" + user
            is_phone = True
            print(f"[{format_now()}] 登录类型：手机号登录（自动添加+86）")
    else:
        is_phone = True
        print(f"[{format_now()}] 登录类型：手机号登录")

    print(f"[{format_now()}] 发送登录请求...")
    access_token, msg = login_access_token(user, password)
    if access_token is None:
        print(f"[{format_now()}] 登录失败：{msg}")
        return

    print(f"[{format_now()}] 登录成功，获取登录token...")
    print(f"[{format_now()}] access_token：{access_token}")
    login_token, app_token, user_id, msg = grant_login_tokens(
        access_token, device_id, is_phone
    )
    if login_token is None:
        print(f"[{format_now()}] 获取token失败：{msg}")
        return

    print(f"[{format_now()}] login_token：{login_token}")
    print(f"[{format_now()}] app_token：{app_token}")
    print(f"[{format_now()}] user_id：{user_id}")
    print(f"[{format_now()}] 提交步数修改请求...")

    ok, msg = post_fake_brand_data(step, app_token, user_id)

    if ok:
        print(f"[{format_now()}] 修改步数成功！当前步数：{step}")
    else:
        print(f"[{format_now()}] 修改步数失败：{msg}")
